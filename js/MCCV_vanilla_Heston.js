...
var CMC_StdErr, CMC_CompTime, CV_StdErr, CV_CompTime;
...
function CallCV( b, alpha, sigma, s0, K, r, v0, T, rho, n, m){
    //clock_t start;
	//var duration;
	//start = clock();
	var start = new Date().getTime();
	...
	var tmp = new RNG();
   for( var j = 1; j <= m; j++){
	   var s, v, diff, tempmean, z1, z2, scv;
	   s=s0;
	   v=v0;
	   scv =s0;
	   for( var i = 1; i <= n; i++){
		   z1= tmp.GetNormal();
		   z2= tmp.GetNormal();
		   v = Math.max( v + ( alpha*(b - v) )*((T*i)/n - (T*(i - 1))/n) + sigma*Math.sqrt(v*((T*i)/n - (T*(i - 1))/n))*(rho*z1 + Math.sqrt(1-rho*rho)*z2), 0.0000001 );
		   s = s*Math.exp( (r - v/2)*((T*i)/n - (T*(i - 1))/n) + Math.sqrt(v*( (T*i)/n - (T*(i - 1))/n))*z1 );
		 scv = scv*Math.exp( (r - v0/2)*((T*i)/n - (T*(i - 1))/n) + Math.sqrt(v0*( (T*i)/n - (T*(i - 1))/n))*z1 );
	   }
	   diff = Math.exp(-r*T)*Math.max(0.0, s-K) - Math.exp(-r*T)*Math.max(0.0, scv-K);
   ...
   //duration = (clock() - start) / (var) CLOCKS_PER_SEC;
   var end = new Date().getTime();
   CV_StdErr = Math.sqrt(var_diff /m);   
   CV_CompTime = (end - start) /1000.0;
...
function Calculate(){
    ...
    document.isForm.c.value = CallCV( b, alpha, sigma, s0, K, r, v0, T, rho, n, m);
    document.isForm.cmc.value = Call( b, alpha, sigma, s0, K, r, v0, T, rho, n, m);
    document.isForm.VarCV.value= CV_StdErr;
    document.isForm.VarCMC.value= CMC_StdErr;
    document.isForm.ComCV.value= CV_CompTime;
...
function Reset(){
    ...
    document.isForm.m.value="5000";
   	document.isForm.c.value ="";  //CV
   	document.isForm.cmc.value ="";  //Crude MC
    document.isForm.VarCV.value ="";  //CV
   	document.isForm.VarCMC.value ="";  //Crude MC
    document.isForm.ComCV.value ="";  //CV
...
/*
function CallAV(b, alpha, sigma, s0, K, r, v0, T, rho, n, m){
    //clock_t start;
	//var duration;
	//start = clock();
	
	var callvalue, var_payoff;
	callvalue = 0.0;
	var_payoff= 0.0;
	var tmp = new RNG();
	
   for( int j = 1; j <= m; j++){
	   var s, v, payoff, tempmean, z1, z2, sav;
	   s = s0;
	   v = v0;
	   sav = s0;
	   for( int i = 1; i <= n; i++){
		   ...
		   v = Math.max( v + ( alpha*(b - v) )*((T*i)/n - (T*(i - 1))/n) + sigma*Math.sqrt(v*((T*i)/n - (T*(i - 1))/n))*(rho*z1 + Math.sqrt(1-rho*rho)*z2), 0.0000001 );
		   s = s*Math.exp( (r - v/2)*((T*i)/n - (T*(i - 1))/n) + Math.sqrt(v*( (T*i)/n - (T*(i - 1))/n))*z1 );
		 sav = sav*Math.exp( (r - v/2)*((T*i)/n - (T*(i - 1))/n) + Math.sqrt(v*( (T*i)/n - (T*(i - 1))/n))*(-z1) );
	   }
	   payoff = Math.exp(-r*T)*( Math.max(0.0, s-K) + Math.max(0.0, sav-K) ) /2;
	   tempmean = callvalue;
	   callvalue =  callvalue + (payoff-callvalue)/j;
	   if (j==1){ 
		   var_payoff =  0.0;
...
function BSCallIS( sigmaS, s0, K, r, T, n, m){
	//clock_t start;
	//var duration;
	//start = clock();
    ...
    for( int j = 1; j <= m; j++){
	   var sBS,tempThetaBS, Q, payoff, tempmean, z1;
	   sBS = s0;
	   Q = 1.0;
	   for( int i = 1; i <= n; i++){
		   ...
		   Q = Q*Math.exp( tempThetaBS*z1*Math.sqrt((T*i)/n - (T*(i - 1))/n) - tempThetaBS*tempThetaBS*((T*i)/n - (T*(i - 1))/n)/2 );
	   }
	   payoff = Math.exp(-r*T)*Q*Math.max(0.0, sBS-K);
	   tempmean = callvalue;
	   callvalue =  callvalue + (payoff-callvalue)/j;
	   if (j==1){ 
		   var_payoff =  0.0;

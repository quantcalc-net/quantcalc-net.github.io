...
//-------------------Initial condition---------------------
   // modify from  for (int j=1; j<=J; j++)// j for x steps
    	for (var j=0; j<=J; j++)// j for x steps
    		{
    		    xj=j*dx;
		      	//modify 1 A(0.5)->A(0)
    		    u[j]=ini(this.A(0, C, r)+xj*(s_max-this.A(0, C, r)),k);
			     //fout2<<u[j]<<endl;
    		}
...
*/
            ...
            rhs[1]=rhs[1]+this.aj(C1x,C2x,Cu,si, dx, dx,r,R, tn, s_max, t, ka, C)*dt/2.0*bL(tn+dt, 0);

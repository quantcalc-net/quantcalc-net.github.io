...
function Calculate(){
    ...
    var beta = 0.5825971579390106702;
    

    // 註解不正確？對答案對了好久
    if( Knock < 0.5 ){  //In
        if( Eta > 0 ){ //Down   這個註解有可能寫反了
            H = H*Math.exp( beta*sigma*Math.sqrt(T/m) );
        }else{ //Up     這個註解有可能寫反了
            H = H*Math.exp( -beta*sigma*Math.sqrt(T/m) );
        }
    }else{ //Out
        if( H<S ){
            if( Eta > 0 )   H = H*Math.exp( beta*sigma*Math.sqrt(T/m) );
            else            H = H*Math.exp( -beta*sigma*Math.sqrt(T/m) );
        }else{
            if( Eta > 0 )   H = H*Math.exp( -beta*sigma*Math.sqrt(T/m) );
            else            H = H*Math.exp( beta*sigma*Math.sqrt(T/m) );
        }
    ...
    if( Knock < 0.5 ){  //In
		...
		document.isForm.priceCall.value = BarrierCallAna;

		document.isForm.pricePut.value = BarrierPutAna;
...
function Reset(){
    ...
    document.isForm.priceCall.value="3";
    document.isForm.pricePut.value="2";
}
